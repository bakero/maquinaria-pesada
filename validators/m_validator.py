...
def validate(script_text: str, episode_id: str,
    ...
    # Capa HARD pre-override: trabaja sobre el texto original con siglas
    # canónicas (LLM, RAG, ...) y exige aposición con comas. Fuente de
    # verdad: campo **ES:** del glosario unificado.
    from validators.shared import glossary_expansion
    results.append(glossary_expansion.check_glossary_term_first_use_expanded(
        parts.full_text))
    # Capa SOFT post-override (complementaria): chequea términos curados
    # ya en forma fonética (`ele ele eme` para LLM, etc.) con ventana de
    # proximidad de 250 chars. Cubre 18 términos hardcoded.
